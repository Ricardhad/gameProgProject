...
const ARROW = preload("res://scenes/player/component/Arrow.tscn")
@onready var arrow_spawn_point = $ProjectileSpawn

func _on_frame_changed():
	if animated_sprite_2d.animation == "attack2":
		if animated_sprite_2d.frame == 11 and not has_spawned_arrow:
			spawn_arrow()
			has_spawned_arrow = true
	elif animated_sprite_2d.animation != "attack2":
		has_spawned_arrow = false  # reset if animation changes
...
func update_attack_hitboxes(current_anim: String):
	if current_anim in ["attack"]:
		attack_area_1.disabled = false
	elif current_anim == "attack1":

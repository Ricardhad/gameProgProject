...
var has_spawned_arrow := false
const charge1 = preload("res://scenes/player/component/charge1.tscn")
const charge2 = preload("res://scenes/player/component/charge2.tscn")
@onready var arrow_spawn_point = $ProjectileSpawn

func _on_frame_changed():
	if animated_sprite_2d.animation == "attack":
		if animated_sprite_2d.frame == 4 and not has_spawned_arrow:
			spawn_charge2()
			has_spawned_arrow = true
	elif animated_sprite_2d.animation == "attack1":
		if animated_sprite_2d.frame == 13 and not has_spawned_arrow:
			spawn_charge1()
			has_spawned_arrow = true
	else:
		has_spawned_arrow = false

func spawn_charge2():
	var arrow_instance = charge2.instantiate()
...
func spawn_charge1():
	var arrow_instance = charge1.instantiate()
	get_parent().add_child(arrow_instance)
	arrow_instance.global_position = arrow_spawn_point.global_position
	arrow_instance.scale = Vector2(0.5, 0.5)

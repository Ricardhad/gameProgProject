	...
	match current_state:
		...
		PlayerState.HEAVY_ATTACK:
			if heavy_atk_charges > 0:
				...
				attack_area_1.disabled = true
				if hud: hud.set_attack_button_pressed(true)
			else: set_state(PlayerState.IDLE)
		PlayerState.HEAL:
...
func handle_normal_movement(delta: float):
	...
	if Input.is_action_just_pressed("heavy_attack"): set_state(PlayerState.HEAVY_ATTACK); return
	...
	if not is_on_floor():
		...
		if can_hang(): set_state(PlayerState.HANG); return

	if not is_on_floor():
		velocity += get_gravity() * delta
		if current_state == PlayerState.JUMP and velocity.y > 0: set_state(PlayerState.FALL)
		elif current_state != PlayerState.JUMP: set_state(PlayerState.FALL)
	else:
		jump_count = 0
		if current_state == PlayerState.JUMP || current_state == PlayerState.FALL: set_state(PlayerState.IDLE)
	...
	if direction:
		...
		if is_on_floor() and current_state == PlayerState.IDLE: set_state(PlayerState.RUN)
	else:
		velocity.x = move_toward(velocity.x, 0, 30)
		if is_on_floor() and current_state == PlayerState.RUN: set_state(PlayerState.IDLE)
...
func handle_dash_state(delta: float):
	...
	if dash_time <= 0:
		velocity.x = 0
		set_state(PlayerState.FALL if not is_on_floor() else PlayerState.IDLE)
...
func handle_hang_state():
	if Input.is_action_just_pressed("jump"): set_state(PlayerState.CLIMB_UP)
	elif Input.is_action_just_pressed("down"): set_state(PlayerState.DROP_DOWN)
...
func _on_animated_sprite_2d_animation_finished():
	match animated_sprite_2d.animation:
		"heal":
			...
			set_state(PlayerState.IDLE)
		"pull_up":
			# Pindahkan player ke atas pijakan secara manual
...
func _update_agility_status():
	if GlobalVar.agi_buff_duration > 0:
		jump_cd = base_jump_cd - GlobalVar.AGI_BUFF_COOLDOWN_REDUCTION
		dash_cd = base_dash_cd - GlobalVar.AGI_BUFF_COOLDOWN_REDUCTION
	else:
...
func update_attack_hitboxes(current_anim: String):
	attack_area_1.disabled = not (current_anim in ["attack", "attack1"])
	attack_area_2.disabled = not (current_anim == "attack2")

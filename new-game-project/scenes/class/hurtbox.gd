...
# Sinyal ini sekarang menjadi satu-satunya tugas penting HurtBox
...
func _on_area_entered(area: Area2D) -> void:
	if area is HitBox:
		# Jika area yang masuk adalah HitBox, dapatkan damage-nya
		var damage_val = area.get_damage()
		
		# Pancarkan sinyal bahwa damage diterima, biarkan skrip lain yang mengurusnya
		received_damage.emit(damage_val)
		
		# Tidak ada lagi baris "health.health -= ..." di sini

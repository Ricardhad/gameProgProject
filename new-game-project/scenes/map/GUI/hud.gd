...
# Variabel untuk label HP
@onready var health_bar_label: Label = $CanvasLayer2/Panel/HealthBar/Label
# Variabel baru untuk Heavy Attack Bar
@onready var heavy_attack_bar: TextureProgressBar = $CanvasLayer2/Panel/CDBar # <--- Ensure this path is correct in your scene
...
func _process(delta: float):
	# Update seluruh HUD, except for heavy attack bar which is updated by player_node via a dedicated function
	_update_hud()
...
func _update_hud():
	...
	# NOTE: heavy_attack_bar is now updated via update_heavy_attack_charges() function
	# called directly from the Player script when charges change.
	# So, you don't need to put heavy_attack_bar.value = player_node.heavy_atk_charges here.


# --- NEW FUNCTION: To explicitly update heavy attack charges ---
# Call this function from your Player script whenever charges change.

# NPC.gd (Versi dengan Toko Acak, Refresh, dan Back)
...
@onready var panel_shop: Panel = $PanelShop

# Referensi node di dalam PanelShop
@onready var button_card_1: TextureButton = $PanelShop/ButtonCard1
@onready var label_detail_1: Label = $PanelShop/ButtonCard1/LabelDetail
@onready var texture_rect_1: TextureRect = $PanelShop/ButtonCard1/TextureRect
@onready var label_coin_1: Label = $PanelShop/ButtonCard1/LabelCoin

@onready var button_card_2: TextureButton = $PanelShop/ButtonCard2
@onready var label_detail_2: Label = $PanelShop/ButtonCard2/LabelDetail
@onready var texture_rect_2: TextureRect = $PanelShop/ButtonCard2/TextureRect
@onready var label_coin_2: Label = $PanelShop/ButtonCard2/LabelCoin

@onready var button_card_3: TextureButton = $PanelShop/ButtonCard3
@onready var label_detail_3: Label = $PanelShop/ButtonCard3/LabelDetail
@onready var texture_rect_3: TextureRect = $PanelShop/ButtonCard3/TextureRect
@onready var label_coin_3: Label = $PanelShop/ButtonCard3/LabelCoin

@onready var button_confirm_buy: TextureButton = $PanelShop/ButtonBuy
@onready var button_refresh: TextureButton = $PanelShop/ButtonRefresh # <- NODE BARU
@onready var button_back: TextureButton = $PanelShop/ButtonBack       # <- NODE BARU
@onready var label_message: Label = $PanelShop/LabelMessage

var player_in_range: bool = false
var selected_card_id: int = 0

# --- STRUKTUR DATA BARU UNTUK BUFF ---
...
var current_shop_items = [] # Menyimpan 3 item yang sedang ditampilkan
...
func _ready():
	...
	button_back.pressed.connect(close_all_ui) # Tombol back langsung memanggil close_all_ui
...
func _on_open_shop_pressed():
	...
	refresh_shop() # <- Muat item acak saat toko pertama kali dibuka
...
# --- FUNGSI LOGIKA TOKO BARU ---

func _on_refresh_pressed():
	# Cek apakah pemain punya koin untuk refresh (opsional)
	# Misalnya, butuh 5 koin untuk refresh
	var refresh_cost = 5
	if GlobalVar.coin_collected >= refresh_cost:
		GlobalVar.coin_collected -= refresh_cost
		print("Toko di-refresh, biaya 5 koin.")
		refresh_shop()
	else:
		show_message("Butuh %d koin untuk refresh!" % refresh_cost)

func refresh_shop():
	# Reset status visual dan logika
	selected_card_id = 0
	button_card_1.visible = true
	button_card_2.visible = true
	button_card_3.visible = true
	button_card_1.button_pressed = false
	button_card_2.button_pressed = false
	button_card_3.button_pressed = false
	
	# Acak daftar buff utama
	all_available_buffs.shuffle()
	
	# Ambil 3 buff teratas untuk ditampilkan
	current_shop_items = all_available_buffs.slice(0, 3)
	
	# Perbarui tampilan setiap kartu
...
func update_card_display(card_slot: int, item_data: Dictionary):
	var label_detail: Label
	var texture_rect: TextureRect
	var label_coin: Label

	match card_slot:
		1:
			label_detail = label_detail_1
			texture_rect = texture_rect_1
			label_coin = label_coin_1
		2:
			label_detail = label_detail_2
			texture_rect = texture_rect_2
			label_coin = label_coin_2
		3:
			label_detail = label_detail_3
			texture_rect = texture_rect_3
			label_coin = label_coin_3
	
	label_detail.text = item_data.desc
	texture_rect.texture = load(item_data.path)
	label_coin.text = str(item_data.cost)

# --- FUNGSI PEMILIHAN & PEMBELIAN ---
...
func _on_confirm_purchase_pressed():
	...
	# Ambil data item yang dipilih dari daftar saat ini
